    ...
    for iter = 1:maxiter 
        ...
        for i=1:length(v)
            ...
            elseif i==length(v)
               ...
               v(i)=max( (1-omega)*v_old(i)+omega*iter_rhs/A(i,i), K-S0*exp(x_grid(i)) ); % put 
            %    v(i)=max( (1-omega)*v_old(i)+omega*iter_rhs/A(i,i), S0*exp(x_grid(i))-K ); % call 
        end
...
fprintf('American Put price under NIG model using PIDE theta method with theta=%d and operator splitting for general Lévy but truncating small jumps',theta)
NIG_american_put_price= interp1(S0*exp(x_grid), v, S0, 'spline')
