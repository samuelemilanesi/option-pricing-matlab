function [S,SAV,ST,STAV]=Kou_simulate_asset_AV(par,Nsim,M)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    X=zeros(Nsim,M+1); XAV=zeros(Nsim,M+1); Z=randn(Nsim,M);
    for i=1:Nsim
        ...
        for j=1:M
            ...
            for jj=1:NT(i)
                if (JumpTimes(jj)>(j-1)*dt).*(JumpTimes(jj)<=j*dt)
                    if rand<p
                        u=rand;
                        Jump=icdf('Exponential',u,1/lambdap);
                        JumpAV=icdf('Exponential',1-u,1/lambdap);
                    else
                        u=rand;
                        Jump=-icdf('Exponential',u,1/lambdam);
                        JumpAV=-icdf('Exponential',1-u,1/lambdam);
                    end
                    X(i,j+1)=X(i,j+1)+Jump;
                    XAV(i,j+1)=XAV(i,j+1)+JumpAV;
                end
            end
        end
    end
