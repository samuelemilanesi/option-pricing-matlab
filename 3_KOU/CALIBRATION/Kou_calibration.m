%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   Calibrate Kou model parameters on a set of vanilla options 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear; close all;
...
par = struct('S0',S0,'r',r,'TTM',0,'sigma',0,'p',0,'lambdap',0,'lambdam',1,'lambdaK',1);
t=tic();

%% RSS loss minimization 
gaps = @(x) fun_Kou_model_mkt_gap(par,x,Strikes, TTMs, mkt_prices);

% Params optimization
options = optimoptions('lsqnonlin','FunctionTolerance',1e-8,'OptimalityTolerance',1e-8,'StepTolerance',1e-8);
opt_params = lsqnonlin(@(x) gaps(x), [0.3 0.5 3 10 10],[0 0 0 1 1], [0.5 1 100 1000 1000], options)    
toc(t)
%% Error and plot
figure
plot(Strikes, mkt_prices,'+'); hold on
[err_vect, model_prices] = fun_Kou_model_mkt_gap(par,opt_params,Strikes, TTMs,mkt_prices);
Error = norm(err_vect)
plot(Strikes, model_prices,'s'); 
legend('Market price', 'Kou price')