%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Price American Put option under extNIG model
...
% upper_boundary_condition = @(t,x) S0*exp(x)-K;                       % v(x,t) for x>xmax
...
N = 500; dx = (xmax-xmin)/N; x_grid = linspace(xmin,xmax, N+1)';   % space grid
...
for j = (M-1):-1:0
    ...
    for iter = 1:maxiter 
        ...
        for i=1:length(v)
            ...
            % v(i)=max( (1-omega)*v_old(i)+omega*iter_rhs/A(i,i), S0*exp(x_grid(i))-K );
        end
...
fprintf('American Put price under extNIG model using PIDE theta method with theta=%d and operator splitting for general Lévy but truncating small jumps',theta)
extNIG_american_put_price= interp1(S0*exp(x_grid), v, S0, 'spline')
